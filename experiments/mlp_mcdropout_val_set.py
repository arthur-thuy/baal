...
# wrapper.add_metric("accuracy", lambda: torchAccuracy().to(device))
...
for step in range(2):
    ...
    test_loss = wrapper.test_on_dataset(test_ds, batch_size=32, use_cuda=use_cuda, eval_set="val")
...
pprint(wrapper.get_metrics("val"))
pprint(wrapper.metrics["val_loss"].value)
